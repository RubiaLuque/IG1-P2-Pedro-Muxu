...
void ResultState::onEnter() {
    Text* title = new Text(game, glm::vec2(ofGetWidth() / 2, 0.7 * ofGetHeight() / 3), 100, "You Win!", game->getFont(), ofColor::green);
    title->setOrigin(glm::vec2(0.5, 0.5));
    game->addGameObject(title);

    Text* results = new Text(game, glm::vec2(ofGetWidth() / 2, 1.4 * ofGetHeight() / 3), 60, "Best results:", game->getFont(), ofColor::white);
    ...
    for (int i = 0; i < times.size(); ++i) {
        ...
        Text* timeResult = new Text(game, glm::vec2(ofGetWidth() / 2, 1.8 * ofGetHeight() / 3 + 60 * i), 45, result, game->getFont(), ofColor::white);
